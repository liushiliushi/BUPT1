...
void bestway(int num)//优化路线函数，num为所需优化路线的骑手序号 
{
	...
	if(number>=6)//当前任务点数大等于6个 
	{
									...
									for(e=0;e<6;e++)//第五个任务点序号是e 
									{
										if(e!=a&&e!=b&&e!=c&&e!=d)//a,b,c,d,e各不相同 
										{
											for(f=0;f<6;f++)//第六个任务点序号是f 
											{
												...
												{
													allpath6[i]=(struct pathpoint **)malloc(sizeof(struct pathpoint *)*7);//allpath[i]为第i种路线，里面是六个指向任务点的指针 
		...
		for(i=0,timemin=100000;i<720;i++)//判断720种路线是否可行并择出最优 
		{
			...
			for(j=0;j<6&&allpath6[i]!=NULL;j++)//判断第i种路线是否有一个订单中食客在餐馆前面 
			{
				if(allpath6[i][j]->order->current==1&&allpath6[i][j]->n==0)//该任务点属于待取餐订单并且是餐馆 
				{
					...
					{
						if(allpath6[i][k]->order->current==1&&allpath6[i][k]->n==1&&allpath6[i][j]->order==allpath6[i][k]->order)//该任务点是否和上述任务点同属一个订单并且是食客 
						{
							temp=k;//记录该任务点序号 
						}
					}
					if(allpath6[i][temp]->order->current==1&&allpath6[i][temp]->n==1&&allpath6[i][j]->order==allpath6[i][temp]->order)//该任务点是否和上述任务点同属一个订单并且是食客
					{
						if(temp<j) //是否食客在餐馆前面 
						{
							timemin=150000;
			...
			{
				timer=(abs(allriders[num].x-allpath6[i][0]->x)+abs(allriders[num].y-allpath6[i][0]->y))/2;//骑手到i号路线第一个任务点所需花费时间 
				allpath6[i][0]->time=time+timer;//计算第i号路线第一个任务点到达时间 
				for(j=0;j<5;j++)//计算其余任务点到达时间 
				{
					timer+=((abs(allpath6[i][j]->x-allpath6[i][j+1]->x)+abs(allpath6[i][j]->y-allpath6[i][j+1]->y))/2);//计算从第j个任务点到底j+1个任务点所需时间 
		...
		if(number==0) allriders[num].finaltask=allriders[num].temporarytask[0];//没有路线正常时选第一个单为目标点 
	...
	else if(number==2)//当前有两个任务点 
	{
	    ...
	    {
	        a=((abs(allriders[num].x-allpathpoint[0].x)+abs(allriders[num].y-allpathpoint[0].y))/2+time);//先去0号再去1号时到达0号的时间 
	        b=((abs(allpathpoint[1].x-allpathpoint[0].x)+abs(allpathpoint[1].y-allpathpoint[0].y))/2+a);//先去0号再去1号时到达1号的时间 
	        c=((abs(allriders[num].x-allpathpoint[1].x)+abs(allriders[num].y-allpathpoint[1].y))/2+time);//先去1号再去0号时到达1号的时间  
		    d=((abs(allpathpoint[0].x-allpathpoint[1].x)+abs(allpathpoint[0].y-allpathpoint[1].y))/2+c);////先去1号再去0号时到达1号的时间  
		    if(b<d||b==d)//是否第一种路线比第二种路线快 
		    {
		    	...
		    	else if(c<allpathpoint[1].order->time+LIMIT&&d<allpathpoint[0].order->time+LIMIT)//当第一种路线超时时是否第二种路线不超时 
			...
			else if(c<allpathpoint[1].order->time+LIMIT&&d<allpathpoint[0].order->time+LIMIT)//当第二种路线快时是否第二种路线不超时 
				allriders[num].finaltask=allriders[num].temporarytask[1];//1号任务点的订单指针赋给骑手当前任务点
	...
	else if(number==3)//当前有三个任务点 
	{
			...
			for(b=0;b<3;b++)//第二个任务点序号为b
			{
				...
				{ 
				    for(c=0;c<3;c++)//第三个任务点序号为c
					{
				        if(c!=a&&c!=b)//c和b和a都不同
						{
							allpath3[i]=(struct pathpoint **)malloc(sizeof(struct pathpoint *)*4);//allpath[i]为第i种路线，里面是四个指向任务点的指针
		...
		for(i=0,timemin=100000;i<6;i++)//判断6种路线是否可行并择出最优
		{
			...
			for(j=0;j<3&&allpath3[i]!=NULL;j++)//判断第i种路线是否有订单中的食客在餐馆前面
			{
				if(allpath3[i][j]->order->current==1&&allpath3[i][j]->n==0)//该任务点属于待取餐订单并且是餐馆
				{
					...
					{
						if(allpath3[i][k]->order->current==1&&allpath3[i][k]->n==1&&allpath3[i][j]->order==allpath3[i][k]->order)//该任务点是否和上述任务点同属一个订单并且是食客
						{
							temp=k;//记录该任务点序号 
						}
					}
					if(allpath3[i][temp]->order->current==1&&allpath3[i][temp]->n==1&&allpath3[i][j]->order==allpath3[i][temp]->order)//该任务点是否和上述任务点同属一个订单并且是食客
					{
						if(temp<j)//是否食客在餐馆前面 
						{
							timemin=150000;
			...
			{
				timer=(abs(allriders[num].x-allpath3[i][0]->x)+abs(allriders[num].y-allpath3[i][0]->y))/2;//骑手到i号路线第一个任务点所需花费时间
				allpath3[i][0]->time=time+timer;//计算第i号路线第一个任务点到达时间
				for(j=0;j<2;j++)//计算其余任务点到达时间
				{
					timer+=((abs(allpath3[i][j]->x-allpath3[i][j+1]->x)+abs(allpath3[i][j]->y-allpath3[i][j+1]->y))/2);//计算从第j个任务点到底j+1个任务点所需时间 
		...
		else allriders[num].finaltask=allpath3[number][0]->order;//最佳路线的第一个任务点的订单指针赋给骑手当前目标点
	...
	else if(number==4)//当前有四个任务点 
	{
				    ...
				    for(c=0;c<4;c++)//第三个任务点序号为c
					{
				        if(c!=a&&c!=b)//c和b和a都不同
						{
							for(d=0;d<4;d++)//第四个任务点序号为d 
							{
								...
								{
									allpath4[i]=(struct pathpoint **)malloc(sizeof(struct pathpoint *)*5);//allpath[i]为第i种路线，里面是五个指向任务点的指针 
		...
		for(i=0,timemin=100000;i<24;i++)//判断24种路线是否可行并择出最优
		{
			...
			for(j=0;j<4&&allpath4[i]!=NULL;j++)//判断第i种路线是否有订单中的食客在餐馆前面
			{
				if(allpath4[i][j]->order->current==1&&allpath4[i][j]->n==0)//该任务点属于待取餐订单并且是餐馆
				{
					...
					{
						if(allpath4[i][k]->order->current==1&&allpath4[i][k]->n==1&&allpath4[i][j]->order==allpath4[i][k]->order)//该任务点是否和上述任务点同属一个订单并且是食客
						{
							temp=k;//记录该任务点序号 
						}
					}
					if(allpath4[i][temp]->order->current==1&&allpath4[i][temp]->n==1&&allpath4[i][j]->order==allpath4[i][temp]->order)//该任务点是否和上述任务点同属一个订单并且是食客
					{
						if(temp<j) //是否食客在餐馆前面
						{
							timemin=150000;
			...
			if(timemin!=150000)//是否i号路线未废除
			{
				timer=(abs(allriders[num].x-allpath4[i][0]->x)+abs(allriders[num].y-allpath4[i][0]->y))/2;//骑手到i号路线第一个任务点所需花费时间
				allpath4[i][0]->time=time+timer;//计算第i号路线第一个任务点到达时
				for(j=0;j<3;j++)//计算其余任务点到达时间
				{
					timer+=((abs(allpath4[i][j]->x-allpath4[i][j+1]->x)+abs(allpath4[i][j]->y-allpath4[i][j+1]->y))/2);//计算从第j个任务点到底j+1个任务点所需时间 
		...
		else allriders[num].finaltask=allpath4[number][0]->order;//最佳路线的第一个任务点的订单指针赋给骑手当前目标点
	...
	else if(number==5)//当前有五个任务点
	{
							...
							for(d=0;d<5;d++)//第四个任务点序号为d
							{
								if(d!=a&&d!=b&&d!=c)//d和c和b和a都不同
								{
									for(e=0;e<5;e++)//第五个任务点序号为e
									{
										...
										{
											allpath5[i]=(struct pathpoint **)malloc(sizeof(struct pathpoint *)*6);//allpath[i]为第i种路线，里面是六个指向任务点的指针
		...
		for(i=0,timemin=100000;i<120;i++)//判断120种路线是否可行并择出最优
		{
			...
			for(j=0;j<5&&allpath5[i]!=NULL;j++)//判断第i种路线是否有订单中的食客在餐馆前面
			{
				if(allpath5[i][j]->order->current==1&&allpath5[i][j]->n==0)//该任务点属于待取餐订单并且是餐馆
				{
					...
					{
						if(allpath5[i][k]->order->current==1&&allpath5[i][k]->n==1&&allpath5[i][j]->order==allpath5[i][k]->order)//该任务点是否和上述任务点同属一个订单并且是食客
						{
							temp=k; //记录该任务点序号 
						}
					}
					if(allpath5[i][temp]->order->current==1&&allpath5[i][temp]->n==1&&allpath5[i][j]->order==allpath5[i][temp]->order)//该任务点是否和上述任务点同属一个订单并且是食客
					{
						if(temp<j) //是否食客在餐馆前面
						{
							timemin=150000;
			...
			if(timemin!=150000)//是否i号路线废除
			{
				timer=(abs(allriders[num].x-allpath5[i][0]->x)+abs(allriders[num].y-allpath5[i][0]->y))/2;//骑手到i号路线第一个任务点所需花费时间
				allpath5[i][0]->time=time+timer;//计算第i号路线第一个任务点到达时间
				for(j=0;j<4;j++)//计算其余任务点到达时间
				{
					timer+=((abs(allpath5[i][j]->x-allpath5[i][j+1]->x)+abs(allpath5[i][j]->y-allpath5[i][j+1]->y))/2);//计算从第j个任务点到底j+1个任务点所需时间 
					allpath5[i][j+1]->time=timer+time; //计算到第j+1个任务点的时间 
				}
				for(j=0;j<5&&allpath5[i]!=NULL;j++)//遍历第i条路线所有任务点，判断其是否超时
				{
					if(allpath5[i][j]->order->time+LIMIT<=allpath5[i][j]->time)//第i条路线第j个任务点是否超时
					{
						timemin=150000;
					}
	            }
		...
		else allriders[num].finaltask=allpath5[number][0]->order;//最佳路线的第一个任务点的订单指针赋给骑手当前目标点
